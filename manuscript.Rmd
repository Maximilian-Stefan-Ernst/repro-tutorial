...
Preregistration increases the credibility of empirical results and is generally accepted as best practise in (psychological) research.
<!--ME nobody cares/ everybody knows the benefits of preregistration-->

<!--ME start with the problem, finish with the solution-->
However, preregistration can be ambiguous. For example, if you write XXX in your preregistration, it is unclear, if XXX, XXX and XXX. <!--ME I believe you have a source for that claim with examples?-->
One could think this problem can be solved by just beeing more precise in the preregistration. However, instead of making preregistration more tedious while probably still keeping some ambiguity, we would like to propose a solution of which we think as unambiguous and comprehensive: using reproducible research projects **with the full analysis code written in advance to data collection**.
We term this approach _preregistration as code (PAC)_. While traditional preregistration entails formulating a planned analysis, Preregistration as Code simply means translating this section into executable computer code, using simulated data.
We believe that while preregistration as Code is unambigous, it can also save the researcher time and resources (see below).
<!--ME I hate it when people write paragraphs about what they are going to say. Just say it.-->

## Benefits of PAC for study planning

We consider three criteria when planning a study, all of which PAC can improve:
* effectiveness - the study can be expected to answer the research question at hand
* efficiency - only require the necessary amount of resources
* comprehensiveness - the research question, study design and analysis are described in detail
<!--ME Maybe even delete those bullet points -->

<!--ME This is not PAC, it is PAC + writing a reproducible Manuscript. I think it is confusing to mix those concepts here. I would delete this section. -->
...
To ensure that the analysis is technically feasible, they should translate their method section into executable computer code for the data analysis using simulated data.
...
We argue that simulating data offers various benefits in the planning phase of a study: <!--ME Why introducing your criteria if you dont really use them? -->
Effectiveness: With PAC, you have to think about and address data analytic problems on a conceptual level before you make the effort to acquire data [@axelrodAdvancingArtSimulation1997]. This can help to prevent costly errors like collecting the wrong variables or ....
Efficiency: If you already have your analysis code, it is straight forward to run repeated simulations with varying parameters. For example, researchers can assess the power (the probability to detect an effect of a given size) for varying numbers of observations in a simulation to determine the required sample size for their study [@brandmaier2018precision]. You may also evaluate how the data analysis copes with expected data complications, e.g. non-normal data [@harrisonIntroductionMonteCarlo2010; @raychaudhuriIntroductionMonteCarlo2008].
Comprehensiveness: With simulated data you can test if your analysis works with missing values, extreme outliers, etc. and write code for coping with those issues. This eliminates the problem of ambiguously preregistered ad-hoc solutions for those problems.
...
<!--ME this does not belong to this section-->
...
To guide the reader through a preregistration as code (PAC), we will use an simple exemplary research question:
...
We suggest writing the PAC three parts: Code that...
...
<!--ME i would give all functions in pseudo-code first-->

_Include pseudo-code here_
...
This is the function we get for our example:
```{r planned_analysis}
```

Obove we argued that preregistration often still is ambiguous, and even in this simple t-test analysis is room for ambiguity:
...
## Simulate Data
_Write two sentences here, and include pseudo-code_
...
To this end, we add dynamic code blocks to our Markdown report that implement the planned reporting of the final results.
...
<!--ME I think this belongs to "Data Simulation"? or maybe not... -->
